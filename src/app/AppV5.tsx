...
function CounterRow({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <div className="counter-row v5-counter-row"><label><span>{label}</span><input type="number" min="0" inputMode="numeric" value={value || ''} placeholder="N/A" onChange={(event) => onChange(Math.max(0, Number(event.target.value) || 0))}/></label><div className="counter-actions"><button type="button" aria-label={`Diminuir ${label}`} onClick={() => onChange(Math.max(0, value - 1))}>−</button><button type="button" aria-label={`Aumentar ${label}`} onClick={() => onChange(value + 1)}>+</button></div></div>;
}
...
export default function AppV5() {
  ...
  return <MotionConfig reducedMotion="user"><main className="app-shell v4-shell v5-shell">
    <header className="topbar v4-topbar v5-topbar"><div className="brand-lockup"><div className="brand-mark">R</div><div><span className="eyebrow">RELATÓRIO INICIAL</span><h1>Início de turno</h1></div></div><span className="product-chip v4-save-chip">{lastSavedAt ? <><Save size={14}/> Salvo</> : 'V5'}</span></header>

    <section className="command-panel v4-route-card v5-route-card"><div className="command-copy"><span>PASSAGEM</span><strong>{shift}º <ArrowRight size={18}/> {selectedNextShift}º</strong><small>{lastSavedAt ? 'Rascunho local protegido' : 'Defina a rota do relatório'}</small></div><div className="route-selectors"><div className="route-selector"><label>Atual</label><ShiftButtons value={shift} disabledValue={selectedNextShift} onChange={selectCurrentShift}/></div><span className="route-arrow"><ArrowRight size={18}/></span><div className="route-selector"><label>Próximo</label><ShiftButtons value={selectedNextShift} disabledValue={shift} onChange={selectNextShift}/></div></div></section>

    <div className="v5-intake-grid">
      <Collapsible.Root open={manualExpanded} onOpenChange={setManualExpanded} asChild>
        <section className="panel manual-panel v4-manual v5-manual" ref={manualRef}>
          <Collapsible.Trigger asChild><button className="manual-panel-toggle" type="button"><div><span className="step-index">01</span><div><h3>Dados manuais do setor</h3><small>{filledCounters ? `${filledCounters} campo(s) preenchido(s)` : 'Preencha antes de colar as mensagens'}</small></div></div><span>{manualExpanded ? <><ChevronUp size={16}/> Recolher</> : <><ChevronDown size={16}/> Abrir</>}</span></button></Collapsible.Trigger>
          <Collapsible.Content><div className="manual-groups v5-manual-groups">{counterGroups.map((group) => <div className="manual-group" key={group.title}><h4>{group.title}</h4><div className="counter-list">{group.items.map(([key, label]) => <CounterRow key={key} label={label} value={counters[key]} onChange={(value) => setCounters((current) => ({ ...current, [key]: value }))}/>)}</div></div>)}<button type="button" className="v5-clear-manual" onClick={clearManual}>Limpar dados manuais</button></div></Collapsible.Content>
        </section>
      </Collapsible.Root>

      <section className="panel intake-panel v4-intake v5-intake"><div className="panel-head compact-head"><div><span className="step-index">02</span><div><h3>Mensagens dos preparadores</h3><small>{snapshot ? 'Fonte do consolidado atual' : 'Cole aqui o conteúdo bruto do WhatsApp'}</small></div></div><div className="head-actions">{raw && <button className="icon-text-action muted-action" type="button" onClick={clearInput}><Trash2 size={15}/> Limpar entrada</button>}{!snapshot && <button className="text-action" type="button" onClick={() => setRaw(demoInput)}>Demonstração</button>}</div></div>
        {snapshot && !inputExpanded && audit ? <div className="input-summary v4-input-summary"><div className="input-summary-stats"><span><strong>{audit.messages}</strong><small>mensagens</small></span><span><strong>{audit.lines}</strong><small>linhas</small></span><span><strong>{audit.sourceMachines}</strong><small>TNLs</small></span></div><div className="input-summary-actions"><button type="button" className="secondary-button compact-button" onClick={() => setInputExpanded(true)}><PencilLine size={15}/> Editar entrada</button><button type="button" className="primary-button compact-button" onClick={analyze}><RefreshCw size={15}/> Reanalisar</button></div></div> : <><textarea className="v5-raw-input" value={raw} onChange={(event) => { setRaw(event.target.value); if (snapshot) setInputExpanded(true); }} placeholder="Cole aqui todas as mensagens copiadas do WhatsApp..." aria-label="Mensagens dos preparadores"/><div className="intake-footer"><div className="intake-status"><span className={rawLineCount ? 'live-dot active' : 'live-dot'}/><span>{inputDirty ? 'Entrada alterada · reanálise necessária' : rawLineCount ? `${rawLineCount} linhas capturadas` : 'Aguardando mensagens'}</span></div><button className="primary-button" type="button" onClick={analyze} disabled={!raw.trim()}>{snapshot ? 'Reanalisar' : 'Analisar setor'} <ArrowRight size={16}/></button></div></>}
      </section>
    </div>

    {snapshot && audit && <section className="analysis-stack v4-analysis v5-analysis" ref={resultRef}>
